...
void readFile(const char *filename)
{
    ...
    if (file == NULL)
    {
        printf("Cannot open nguoidung.txt!!!");
        exit(1);
    }

    eType x;
    while (fscanf(file, "%s %s %d", x.username, x.password, &x.status) != EOF)
    {
        InsertNode(x, &T);
    }
...
// void writeToFile(Tree_T *Root, const char *filename)
// {
...
//         fprintf(file, "%s %s %d\n", (*Root)->data.username, (*Root)->data.password, (*Root)->data.status);
