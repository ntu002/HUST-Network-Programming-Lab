...
Tree_T Search(char userName[], char passWord[], Tree_T Root){ 
	if (Root == NULL) return NULL; // not founsd
...
Tree_T SearchPassword(char x[],Tree_T Root){ 
	if (Root == NULL) return NULL; // not founsd
		...
		return Root; 
	else return SearchPassword(x,Root->right); 
}
    ...
    if (*Root == NULL){
	 /* Create a new node for key x */ 
        *Root=(node_Type*)malloc(sizeof(node_Type)); 
        ...
        (*Root)->right = NULL;
        (*Root)->data.count = 0;
        if((*Root)->data.status == 2) strcpy((*Root)->data.code, "20205135");
        // (*Root)->data.inp[0] = inp;
        // (*Root)->data.out[0] = out; 
    } else 
        ...
        if (strcmp(x.username, (*Root)->data.username) < 0) InsertNode(x, &(*Root)->left);
        else 
            if (strcmp(x.username, (*Root)->data.username) >0) InsertNode(x, &(*Root)->right); 
} 
...
void inorderprint(Tree_T T)
{
    ...
    {
        inorderprint(T->left);
            printf("%-20s%-10s%-10d%-10s\n\n", T->data.username , T->data.password, T->data.status, T->data.homepage);
